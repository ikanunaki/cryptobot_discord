...
#       return round(float(json.loads(req.text)['balance']),CONFIG['COIN']['decimals'])
        return round(float(req.text),CONFIG['COIN']['decimals'])
...
def mostrar_saldo():
    ...
    for mn in my_addresses:
        MN_Current_Coins = get_saldo(mn['explorbalance'])
...
def mostrar_inversores():
    ...
    for mn in my_addresses:
        ...
        embed.description += "Generado:" + " {0:.{1}f}".format(MN_Generated, 1) + " {0:.{1}f}".format(MN_Generated + float(CONFIG['COIN']['collateral']) + float(mn['setup_balance']) , 1) + '\n' 
...
LISTA_COMANDOS = {
  ...
  "PRECIO":             "Muestra el precio en CoinMarketCap de PIRL",
  "SALDO":              "Muestra el saldo actual total",
...
async def on_message(message):
    if (message.content.upper()[0:5]=="/BIT "):
